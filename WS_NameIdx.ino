...
char * drawDayDate()
  {
     ...
     //sprintf(out,"%s %s,%02d-%02d-%04d   %02d %s %dH\0",DayName(daynow),pasar[jumlahhari()%5],now.day(),now.month(),now.year(),nowH.hD,locBuff,nowH.hY);
      sprintf(out,"%s, %02d %s %04d    %02d %s %dH\0",DayName(daynow),now.day(), MonthName(now.month()),now.year(),nowH.hD,locBuff,nowH.hY);
      return out;
  }
